...
<a name="status-version"></a>
## 8. "Status: 0.4.0"——把 registry 上的版本当成了本仓库的版本

**当时写的**：README 两处写 `**Status**: 0.4.0`；而 `pyproject.toml`、`__init__.py`、`CHANGELOG.md`、
`CITATION.cff` 与站点都在说 0.4.1。

**错在哪**：把两件不同的事实混成一句。PyPI 上确实只有 0.4.0（那个 patch 没发出去），而**这份代码**是
0.4.1。读者看到"这个库是 0.4.0"，会以为装到的与文档一致——恰好相反：`pip install paired-eval` 装到的是
**更旧**的库。

**修正**：状态行改为 `0.4.1（PyPI 上仍是 0.4.0）`；新增 `.github/workflows/release.yml`，tag 与
`pyproject.toml` 版本不一致时**拒绝发布**；`claims.json` 增加"包自报版本 == manifest 版本"，每次 CI 复算。

**机制化**：一句"状态"里含两个会被混淆的事实，就必须分别可核对——一个是包自报（可进 CI 门禁），
一个是 registry 现状（只能人工核对，因此写在括号里并注明日期）。

---

<a name="latency-estimate"></a>
## 9. "~20 s"——把估算写成了实测

**当时写的**：README 的 `required_pairs(0.15, 0.30)` 注释写着 "runs the real permutation test (~20 s)"。

**错在哪**：20 秒是记忆里的数字，没有任何机器上的记录。实测（默认 sims=200 / resamples=2000，
i7-10700）是 **71 秒**；复核方在另一台机器上独立测得 73.5 秒。方法那半句是真的——它确实跑真置换检验——
但耗时被低估约 3.5 倍，而这是读者决定"要不要现在跑"的唯一依据。

**修正**：改为 "about a minute: 71 s on an i7-10700; drop sims/resamples for a quick estimate"；
`claims.json` 增加一条把这句话钉在代码参数上：默认 `sims=200, resamples=2000`——改默认值就会改这句话的含义。

**机制化**：耗时类数字只有两种诚实写法——写清机器与日期（"71 s on an i7-10700, 2026-09"），
或者只给量级并同时给出可调参数。把估算写成实测，是这份文档里最难自查的一类错误。
