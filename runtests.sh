#!/bin/sh
# paired-eval 全套件入口。任何套件失败即非零退出 —— pre-commit 钩子与 CI 共用。
...
for f in tests/test_*.py; do
    ...
    if [ "$mains" -ne 1 ]; then
        echo "[$(basename "$f")] FAILED: __main__ 块数量为 $mains, 必须恰好 1"
    ...
    if out=$(python3 -B "$f" 2>&1); then
        ...
        echo "[$(basename "$f")] $n tests passed"
        files=$((files + n))
    else
...
if [ "$nfiles" -eq 0 ]; then
    echo "=== 套件失败: tests/ 下未找到任何 test_*.py(空套件不算通过) ==="
    exit 1
fi
if [ "$fail" -ne 0 ]; then
    echo "=== 套件失败:$bad ==="
