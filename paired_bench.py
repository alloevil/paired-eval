...
harness x agent 的 2x2(同一批题、同一次交错运行, 3 轮):
  ...
  当时的结论(已被第116轮推翻, 保留以留痕): "两个因子是替代品而非互补品, 任一到位即够,
  两者都做是浪费"。这个读法基于 strict 侧两格都是 1.000 的观察, 而那是天花板 ——
  换到有余量的题型后, 严格提示上叠加自检仍有 +0.183(p=0.0046)的增量。见下方第116轮。
  ...
  实践含义(第116轮修正后): 脚手架更便宜(只多一段前缀 vs 多一次模型调用), 故应先做;
  但"做完脚手架就不必加自检"是错的 —— 有余量的题型上自检仍加 +0.183(p=0.0046)。
  第114轮当时只敢说"叠加无收益未经证实", 这个谨慎是对的: 一轮后它确实被证伪。
机械化(第114轮): report() 现在自动报出触顶/触底系统, 并声明"这些系统之间的 null 与
  任何交互项都不可解释"。上轮的报告没提天花板 —— 靠人记得, 就会有忘的时候。

替代关系在第二个任务族上复现(第115轮, 按第114轮的处方"换任务族"):
  忠实性轴的 2x2(3 个案例 x {宽松提示"补充背景", 严格提示"仅依据资料"} x {单遍, 自检删无据}),
  评分口径 = must_ground 的 grounding_rate:
                loose    tight
    single      0.089    1.000
    check       1.000    1.000
  harness 主效应 +0.455 | agent 主效应 +0.455 | 交互项 -0.911
  动态范围 11.2x(格式轴只有 3.0x) —— 基线远离天花板, 故"任一因子单独即够"这半的
  证据比格式轴更强; 但三格仍触顶, 交互那半依旧不可测(第三次尝试失败)。
  机制证据(比分数更硬): claim 数从 138 条(115 无据)压缩到 12 条(0 无据), 而严格提示
  下同样是每案 2 条 = 资料的事实条数。两条路径产出的不只是同样的分数, 而是同样的
  输出构成 —— 自检确实是在删掉无据内容, 不是在改写措辞骗过 judge。
"纯替代"结论被推翻(第116轮, 六轮筛选后终于拿到不触顶的参照格):
  第113/114轮在格式轴与忠实性轴都测出"任一因子到位即够", 但两次的参照格都在 1.000,
  交互那半不可测。本轮改造题型找到有余量的形态 —— **跨条目推算**: 资料只给终值与
  增长率/毛利率/两端年份, 问初值/毛利/年均增速。模型会把自己算出的数当成资料所载,
  故严格提示下也只有 0.64~0.89(n=3 复核稳定, 不触顶)。
  2x2(3 题 x 2 轮起, 关键格补到 6 轮):
                single   check
    loose        0.396    0.807
    tight        0.794    0.917
  harness 主效应 +0.254 | agent 主效应 +0.266 | 交互项 -0.288
  关键读数 —— 严格提示已到位时自检的增量: Δ=+0.183 CI95=[+0.086,+0.285] p=0.0046
    (18 个配对单元)。若两因子是纯替代, 这个数应为 0; 实测显著为正。
  结论: 两者是**收益递减但可叠加**, 不是替代。宽松提示下自检加 +0.411, 严格提示下
    仍加 +0.183 —— 后者小得多(故交互项为负), 但不是零。
  修正的实践含义: "选脚手架还是加自检"不是二选一; 脚手架更便宜应先做, 但做完之后
    自检仍有可测的额外收益。第113/114轮说"任一到位即够"是天花板造成的假象。
  独立复现(第117轮, reproduce_findings.check_derivation_increment, 全新一批 18 单元):
    参照格 tight-single = 0.719(仍未触顶), 自检后 = 1.000
    增量 Δ=+0.281 CI95=[+0.195,+0.367] p=0.0002 —— 比记录值 +0.183 更强, 方向一致。
    这条结论现在是三个复现检查之一(另两个是脚手架效应与模型 null), 五条判据分别管:
    增量消失 / 方向反转 / 参照格触顶(处方是换题) / 样本不足(仅警告) / 增量衰减。
过程记录(第116轮, 差点第三次过度声称): 首轮 6 单元时关键增量是 +0.122 但 p=0.496、
  CI=[-0.056,+0.317] 含 0, 我当时已准备写"推翻替代结论"。按点估计与 sd 反算所需样本
  (1.96*sd/Δ)^2 = 18 单元, 补跑 4 轮后才拿到 p=0.0046。教训: 点估计的方向不是结论,
  CI 是否含 0 才是; 而样本量该在看到 CI 之后按需补, 不是先定后测。

手算公式的语义错误(第121轮, 补连续分数规划器时发现): 第116轮我用 (1.96*sd/Δ)^2
  反算样本量, 得 18 对并据此补跑。那个公式答的是"CI 恰好排除 0", 不是"有 80% 功效
  检出" —— 后者是 ((z_{a/2}+z_power)*sd/Δ)^2, 大 2.04 倍。实测置换检验的功效曲线:
    n=18 -> 0.45   n=26 -> 0.60   n=33 -> 0.71   n=37 -> 0.79   n=42 -> 0.83
  即第116轮那次只有约五成把握, 拿到 p=0.0046 有运气成分(补跑后的真实效应 +0.183
  大于当初的点估计 +0.122, 帮了忙)。结论本身经第117轮独立复现(+0.281)站得住,
  但当时的样本量论证是错的。
  修法: claim_eval.required_pairs —— 内部真跑 paired_compare 做网格搜索, 与
  required_tasks 同一原则(规划器与检验器用同一把尺子), 并把 p 地板作为下界。
  它给 Δ=0.122/sd=0.264 的答案是 42 对, 与教科书公式的 37 同量级。
  连带自查出一处新代码的 bug: 每次 sim 复用同一个重采样种子, 会让所有置换检验用
  完全相同的符号翻转模式, 功效估计偏斜。
  工程副作用也记一笔: 这两个测试真跑蒙特卡洛要 20 秒, 把 pre-commit 依赖的快速套件
  从 3 秒拖到 23 秒。处理办法是边界用注入的确定性采样器(留在第 1 层), 真校准改名为
  非 test_ 前缀并挂到 checkall 的第 3 层 —— 与 check_hooks 同层理由。

同类 bug 的系统排查(第120轮): 置换检验那条路径有同样的错。符号翻转对零差值对是
  恒等操作, 故它们不进入置换分布 —— 可达最小 p 是 2/2^(非零差值对数)。实证: 5 对零差
  + 1 对满差, p=1.0000 恰是 p_floor(1), 与 p_floor(6)=0.031 无关。
  最刺痛的一例是第116轮首轮自己的数据: 差值 [0, -0.17, 0.4, 0, 0, 0.5] 只有 3 对非零,
  当时报"p=0.496 不显著", 真相是"非零差值对只有 3 个, 地板 0.25, 检验到不了显著"。
  修法: paired_compare 自报 n_effective, interpret 默认拿它当地板基数。
  三种基数的优先级与措辞(处方各不相同, 说错会让人加错东西):
    显式 floor_n  -> "不一致对"      McNemar: 加轮次或换能拉开差距的题
    n_effective   -> "非零差值对"    置换: 有单元但差值为零, 加轮次未必有用
...
p 值地板(第115轮实测, 会话里第一次撞上): 3 个配对单元时符号翻转检验的最小可能 p 是
  ...
  n=5 -> 0.064, n=6 -> 0.032。补到 6 案例后 Δ=+0.879 p=0.032 才显著。
  教训: 设计阶段就该按 2/2^n <= alpha 定最小案例数(alpha=0.05 -> n>=6), 否则再大的
  效应也拿不到显著性 —— 这与 MDE(能检出多小的效应)是同一件事的另一面。
...
# ------------------------------------------------------------ 跨条目推算(有余量的题型)
# 第116轮的产出: 六轮筛选里唯一一族"严格提示下也不满分"的题。形态是资料只给终值与
# 比率/两端年份, 问初值/毛利/年均增速 —— 模型会把自己算出的数当成资料所载(distorted)。
# 与格式题的关键差别: 参照格落在 0.64~0.89 而非 1.000, 故交互项在这族上可解释。
# 这些题不进 ALL_TASKS: 它们不是 check 可判的单轮题, 而要走 eval_task 的 trajectory
# 路由(claim 抽取 + 逐条 grounding), 判定依赖 judge。放在这里供 harness/agent A/B 复用。
DERIVATION_CASES = [
    {"id": "der-growth-base",
    ...
    {"id": "der-gross-profit",
     ...
     "trap": "营收 x 毛利率 是资料没写的乘积"},
    {"id": "der-cagr",
     "question": "问题: J公司这三年年均增速多少?",
     "observations": ["J公司2020年营收200亿元。", "J公司2023年营收500亿元。"],
