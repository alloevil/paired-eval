# -*- coding: utf-8 -*-
"""paired_bench — 配对 A/B 的任务集与工作流(全程序判定, 零第三方依赖)。
...
入口:
    ...
    report(reports)                           # 完整诚实报告: 九项一次给全(含 null 界与触顶诊断)
...
题集:
    ALL_TASKS            31 道程序可判定的单轮题(exact / 指令遵循 / 字符 / 格式)
    SCAFFOLD_SENSITIVE   对脚手架敏感的 4 题子集(实测筛出, 见文件末尾)
    DERIVATION_CASES     跨条目推算题(judge 评分, 严格提示下也不满分 —— 唯一有余量的题族)
...
用本任务集测出的结论、方法学教训与被推翻的旧结论, 分别见:
    ...
    docs/lessons.md       方法学教训(通用, 代码注释里的锚点都指向这里)
    docs/corrections.md   纠正记录(原文保留 + 指向推翻它的证据)
"""
...
# ------------------------------------------------------------ 跨条目推算(有余量的题型)
# 六轮筛选(docs/lessons.md#headroom)里唯一一族"严格提示下也不满分"的题。形态是资料只给终值与
...
DERIVATION_CASES = [
    {"id": "der-growth-base",
     ...
     "note": "边缘题(docs/lessons.md#screening-reliability): 真实水平约 0.89~1.0, 两次独立筛选一次留一次弃。"
...
# 筛选协议的可靠性(docs/lessons.md#screening-reliability, 同 6 道候选独立筛两次): 一致率 5/6, 详见 screen_graded
...
def screen_graded(cases, grade, n=2, confirm_n=3, band=(0.0, 0.9), **kw):
    ...
    阶段2(复核, confirm_n 轮): 再跑, 仍在区间内且不是所有轮次都贴同一端才保留。
      复核的必要性与 screen_tasks 同源: 低 n 会把噪声标成"有余量"。实测:
    ...
    默认 band 上界取 0.9 而非 1.0, n 默认 2 —— 两个默认值都来自实测(docs/lessons.md#screening-reliability):
      同 6 道候选被独立筛两次(先手工 n=1, 后用本函数 n=1), 一致率 5/6。
